...
export enum GameState {
  ...
  TEMPLE_HUB, 
  PARTY_MANAGEMENT,
  GAME_WON // Added for Campaign Conclusion
}
...
export enum SpellType { DAMAGE='DAMAGE', HEAL='HEAL', BUFF='BUFF', DEBUFF='DEBUFF', UTILITY='UTILITY', SUMMON='SUMMON' }
...
export interface CombatStatsComponent { 
    ...
    // Transformation Logic (Wild Shape)
    originalStats?: Partial<CombatStatsComponent>; // Backup stats when transformed
    originalSprite?: string; // Backup sprite
}

export interface Item { id: string; name: string; type: 'equipment' | 'consumable' | 'key'; rarity: ItemRarity; description: string; icon: string; flavorText?: string; effect?: any; equipmentStats?: any; }
export interface Spell { id: string; name: string; level: number; range: number; type: SpellType; damageType?: DamageType; diceCount: number; diceSides: number; aoeRadius?: number; aoeType?: 'CIRCLE'|'CONE'|'LINE'; description: string; animation: string; icon: string; summonDefId?: string; }
export interface Skill { id: string; name: string; description: string; staminaCost: number; cooldown: number; range: number; damageMultiplier: number; damageType?: DamageType; diceCount?: number; diceSides?: number; effect: string; statusEffect?: string; isBonusAction?: boolean; aoeRadius?: number; aoeType?: 'CIRCLE'|'CONE'; animation?: string; icon: string; summonDefId?: string; }
